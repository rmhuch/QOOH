...
def make_scaledPots(barrier_height=None, makeplot=False):
    ...
    newVel = Velcoefs * scaling_factor
    ...
    if makeplot:
        ...
        plt.show()
    return scaling_factor, scaledV0, scaledV1, scaledV2
...
def calcHam(V_coeffs, B_coeffs, HamSize=None):
    ...
    if HamSize is None:  # make more flexible to different order sizes - currently only supports 6
        HamSize = 7
...
def make_plots(resDicts, levels=False, wfns=True, title=None):
    for resDict in resDicts:
        ...
        if levels:
            ...
            plt.plot(enX, np.repeat(en0, len(enX)), label=f"{en0}")
            plt.plot(enX, np.repeat(en1, len(enX)), label=f"{en1}")
            plt.plot(enX, np.repeat(en1, len(enX)), label=f"{en2}")
        if wfns:
...
def run(barrier_height=None):
    ...
    if barrier_height is None:
        ...
        make_plots([res0, res1, res2], wfns=True, title="Excitation of OH Unscaled")
    else:
