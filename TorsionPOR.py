...
class POR:
    ...
    def calc_scaled_Vcoeffs(self):
        from FourierExpansions import calc_curves, calc_cos_coefs
        from scaleTORpotentials import scale_barrier
        if len(self.Velcoeffs) != 7:
            ...
            scaling_factor, scaled_energies = scale_barrier(energy_dat, self.barrier_height, self.scaling_factor)
        ...
        for i in np.arange(len(self.Vcoeffs.keys()) - 1):  # loop through saved energies
            scaled_coeffs[f"V{i}"] = newVel + self.Vcoeffs[f"V{i}"]
